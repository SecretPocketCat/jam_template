...
pub fn assets_plugin(app: &mut App) {
    app.add_startup_system(set_window_icon)
        .add_loading_state(
            LoadingState::new(AppState::LoadingSplash).continue_to_state(AppState::Loading),
        )
        .add_collection_to_loading_state::<_, splash::SplashScreenTextureAssets>(
            AppState::LoadingSplash,
        )
        ...
        .add_plugin(ProgressPlugin::new(AppState::LoadingSplash))
        .add_plugin(ProgressPlugin::new(AppState::Loading))
        .fn_plugin(splash::splash_plugin);
}
